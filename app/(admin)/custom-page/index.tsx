...
const fetchCustomPageDetailsById = async (id: number) => {
  try {
    const {data} = await axiosWithAuth.get(`/custom-page-editor/get-custom-page-details`, {
      params: {
        customPageId: id
      }
    });
    ...
    notification.open({
      ...
      message: `customPage`,
      description:
          'Something went wrong while fetching custom page details',
    });
  }
}
...
export default function AddEditCustomPage({id}: IProps) {
  ...
  const onFinish = async (values: any) => {
    ...
    try {
      const res = await axiosWithAuth.post('/custom-page-editor/add-or-modify-custom-page', modifiedValues)
      if (res.status == 200) {
        notification.open({
          type: 'success',
  ...
  return (
      <div className={"p-2 pb-[60px]"}>
        ...
        {((isEditPage && dataCustomPageDetails) || (!isEditPage && dataLanguages)) && <Form
            form={form}
            layout="vertical"
