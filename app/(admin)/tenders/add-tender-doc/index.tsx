...
const fetchTenderDocumentDetailsById = async (id: number) => {
  try {
    const { data } = await axiosWithAuth.get(
      `/tender-editor/get-tender-document-details`,
      {
        params: {
          tenderDocumentId: id,
        },
...
export default function AddEditTenderDoc({ id }: IProps) {
  ...
  const onFinish = async (values: any) => {
    ...
    try {
      const res = await axiosWithAuth.post(
        `/tender-editor/add-or-modify-tender-document`,
        modifiedValues
      );
      if (res.status == 200) {
        notification.open({
          type: "success",
          message: `${
            isEditPage ? "Tender Was Edited" : "Tender Document was added"
          }`,
        });
  ...
  const uploadFile = async (options: any) => {
    ...
    try {
      const res = await axiosWithAuth.post(
        `/tender-editor/upload-tender-document`,
        formData,
        config
      );
      if (res.status == 200) {
  ...
  const getDefaultValue = () => {
    if (isEditPage) {
      const newData = {
        ...
        documentDetails: dataTenderDocumentDetails?.documentDetails.map(
          (detail: any) => ({
            ...detail,
            document: {
      ...
      return {
        ...
        documentDetails: activeLanguages?.map((e) => {
          return {
            id: null,
    ...
    <div className={"p-2 pb-[60px]"}>
      <div className={"w-full flex justify-between items-center mb-4"}>
        ...
        <h2 className={"text-center text-[30px] w-full"}>
          {id ? "Edit Tender Document" : "Add Tender Document"}
        </h2>
      </div>
      <Divider className={"my-3"} />
      {((isEditPage && dataTenderDocumentDetails) ||
              ...
              return (
                <div className={"flex flex-col gap-y-5"}>
                  {fields.map((field, index, c) => {
                    ...
                    const dataFile = form.getFieldValue([
                      "documentDetails",
                      field.name,
                      "document",
                    ]);
                      ...
                      ? [
                          {
                            ...
                            originalFileName:
                              dataTenderDocumentDetails?.documentOriginalName,
                            fileName: dataTenderDocumentDetails?.documentName,
                            contentType:
                              dataTenderDocumentDetails?.documentContentType,
