...
const fetchTenderDetailsById = async (id: number) => {
  try {
    const {data} = await axiosWithAuth.get(`/tender-editor/get-tender-details`, {
      params: {
        tenderId: id
      }
    });
    ...
    notification.open({
      ...
      message: `tender`,
      description:
          'Something went wrong while fetching tender details',
    });
  }
}
...
export default function AddEditTender({id}: IProps) {
  ...
  const onFinish = async (values: any) => {
    ...
    const modifiedValues = {
      ...
      useStartDateInMillis: dayjs(values?.useStartDateInMillis, 'DD-MM-YYYY HH:mm:ss').valueOf(),
      useEndDateInMillis: dayjs(values?.useEndDateInMillis, 'DD-MM-YYYY HH:mm:ss').valueOf(),
    ...
    try {
      const res = await axiosWithAuth.post('/tender-editor/add-or-modify-tender', modifiedValues)
      if (res.status == 200) {
        notification.open({
          type: 'success',
  ...
  const getDefaultValue = () => {
    if (isEditPage) {
      ...
      const newData = {
        ...dataTenderDetails,
        useStartDateInMillis: dataTenderDetails?.useStartDateInMillis ? dayjs.unix(dataTenderDetails.useStartDateInMillis / 1000) : null,
        useEndDateInMillis: dataTenderDetails?.useEndDateInMillis ? dayjs.unix(dataTenderDetails.useEndDateInMillis / 1000) : null,
  ...
  return (
      <div className={"p-2 pb-[60px]"}>
        <div className={"w-full flex justify-between items-center mb-4"}>
          ...
          <h2 className={"text-center text-[30px] w-full"}>{id ? "Edit Tender" : "Add Tender"}</h2>
        </div>
        <Divider className={"my-3"}/>
        {((isEditPage && dataTenderDetails) || (!isEditPage && dataLanguages)) && <Form
            form={form}
            layout="vertical"
        ...
        <Card
            ...
            <Card className={"border-[1px] rounded-2xl border-solid border-[#b2b2b2] flex w-full card-item"}>
                <Form.Item
                    ...
                    label={'Price'}
                >
                    <InputNumber disabled={!status1} placeholder="Enter a number" className="w-full"/>
                </Form.Item>
                <Form.Item
                    className={"!mb-0 !mt-[10px] flex items-center justify-center h-full"}
        ...
        <Card
            ...
            <Card className={"border-[1px] rounded-2xl border-solid border-[#b2b2b2] flex w-full card-item"}>
                <Form.Item
                    ...
                    label={'Price'}
                >
                    <InputNumber placeholder="Enter a number" disabled={!status2} className="w-full"/>
                </Form.Item>
                <Form.Item
                    className={"!mb-0 !mt-[10px] flex items-center justify-center h-full"}
        ...
        <Card
            ...
            <Card className={"border-[1px] rounded-2xl border-solid border-[#b2b2b2] flex w-full card-item"}>
                <Form.Item
                    ...
                    label={'Price'}
                >
                    <InputNumber placeholder="Enter a number" disabled={!status3} className="w-full"/>
                </Form.Item>
                <Form.Item
                    className={"!mb-0 !mt-[10px] flex items-center justify-center h-full"}
        ...
        <Card
            ...
            <Card className={"border-[1px] rounded-2xl border-solid border-[#b2b2b2] flex w-full card-item"}>
                <Form.Item
                    ...
                    label={'Price'}
                >
                    <InputNumber placeholder="Enter a number" disabled={!status4} className="w-full"/>
                </Form.Item>
                <Form.Item
                    className={"!mb-0 !mt-[10px] flex items-center justify-center h-full"}
