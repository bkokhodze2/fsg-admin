...
const fetchSeoPageDetailsById = async (id: number) => {
  try {
    ...
    notification.open({
      ...
      message: `seoPage`,
      description:
          'Something went wrong while fetching seo details',
    });
  }
}
...
export default function AddEditSeoPage({id}: IProps) {
  ...
  const onFinish = async (values: any) => {
    ...
    const modifiedValues = {
      ...
      details: values.details.map((detail: any) => {
        ...
        return ({
            ...
            imageDTO: detail?.imageData ? {...detail.imageData} : {...detail?.imageDTO},
        })
      })
    };
    ...
    try {
      const res = await axiosWithAuth.post('/seo-editor/add-or-modify-seo', modifiedValues)
      if (res.status == 200) {
        notification.open({
          type: 'success',
  ...
  const uploadImage = async (options: any) => {
    ...
    try {
      const res = await axiosWithAuth.post(`/seo-editor/upload-seo-image`, formData, config)
      if (res.status == 200) {
        onSuccess(res.data)
      }
    } catch (e: any) {
  ...
  return (
      <div className={"p-2 pb-[60px]"}>
        <div className={"w-full flex justify-between items-center mb-4"}>
          ...
          <h2 className={"text-center text-[30px] w-full"}>{id ? "Edit SEO" : "Add SEO"}</h2>
        </div>
        <Divider className={"my-3"}/>
        {((isEditPage && dataSeoPageDetails) || (!isEditPage && dataLanguages)) && <Form
            form={form}
            layout="vertical"
              ...
              return <div className={"flex flex-col gap-y-5"}>
                {
                  fields.map((field, index, c) => {
                    ...
                    let fileList = dataImg?.url ? [dataImg] : (dataSeoPageDetails?.details?.[0]?.imageData ? [dataSeoPageDetails?.details?.[0]?.imageData] : []);
