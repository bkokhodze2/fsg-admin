...
const fetchCategories = async () => {
  try {
    const { data } = await axiosWithAuth.get(
      `${BASEAPI}/iv-component-editor/get-iv-component-categories`
    );
    return data;
  } catch (error: any) {
...
const fetchBCardDetailsById = async (id: number) => {
  try {
    const { data } = await axiosWithAuth.get(
      `/iv-component-editor/get-get-iv-component-detail`,
      {
        params: {
          ivComponentId: id,
        },
    ...
    notification.open({
      type: "error",
      message: `BCard`,
      description: "Something went wrong while fetching B card details",
    });
  }
};
...
export default function AddEditBCard({ id }: IProps) {
  ...
  const onFinish = async (values: any) => {
    ...
    try {
      const res = await axiosWithAuth.post(
        "/iv-component-editor/add-or-modify-iv-component",
        modifiedValues
      );
      if (res?.status == 200) {
        notification.open({
  ...
  const uploadImage = async (options: any) => {
    ...
    try {
      const res = await axiosWithAuth.post(
        `/iv-component-editor/upload-iv-component-media-upload`,
        formData,
        config
      );
      if (res?.status == 200) {
    ...
    if (isEditPage && dataBCardDetails) {
      ...
      const newData = {
        ...
        mediaDTO: {
          ...
          originalFileName:
            dataBCardDetails?.mediaDTO?.originalFileName || null,
    ...
    <div className={"p-2 pb-[60px]"}>
      <div className={"w-full flex justify-between items-center mb-4"}>
        ...
        <h2 className={"text-center text-[30px] w-full"}>
          {id ? "Edit B Card" : "Add B Card"}
        </h2>
      </div>
      <Divider className={"my-3"} />
      {((isEditPage && dataBCardDetails) || (!isEditPage && dataLanguages)) && (
        <Form
          form={form}
          layout="vertical"
        ...
        >
          ...
          <Form.Item name={"buttonLink"} label={"buttonLink"}>
