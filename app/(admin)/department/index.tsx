...
const fetchDepartmentDetailsById = async (id: number) => {
  try {
    const { data } = await axiosWithAuth.get(
      `/department-editor/get-department-details`,
      {
        params: {
          departmentId: id,
        },
    ...
    notification.open({
      type: "error",
      message: `department`,
      description: "Something went wrong while fetching department details",
    });
  }
};
...
export default function AddEditDepartment({ id }: IProps) {
  ...
  const onFinish = async (values: any) => {
    ...
    try {
      const res = await axiosWithAuth.post(
        "/department-editor/add-or-modify-department",
        modifiedValues
      );
      if (res.status == 200) {
        notification.open({
  ...
  return (
    <div className={"p-2 pb-[60px]"}>
      ...
      {((isEditPage && dataDepartmentDetails) ||
