...
export default function AddEditManagementPerson({ id }: IProps) {
  ...
  const onFinish = async (values: any) => {
    ...
    try {
      const res = await axiosWithAuth.post(
        "/management-person-editor/add-or-modify-management-person",
        modifiedValues
      );
      if (res.status == 200) {
        notification.open({
  ...
  const uploadImage = async (options: any) => {
    ...
    try {
      const res = await axiosWithAuth.post(
        `/management-person-editor/upload-management-person-image`,
        formData,
        config
      );
      if (res.status == 200) {
    ...
    <div className={"p-2 pb-[60px]"}>
      ...
      {((isEditPage && dataPersonDetails) ||
        ...
        >
          ...
          <Form.Item name={"linkedinUrl"} label={"linkedinUrl"}>
