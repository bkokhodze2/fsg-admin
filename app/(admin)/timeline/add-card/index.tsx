...
const fetchCategories = async () => {
  try {
    const { data } = await axiosWithAuth.get(
      `${BASEAPI}/timeline-editor/get-timeline-categories`
    );
    return data;
  } catch (error: any) {
...
const fetchTimelineDetailsById = async (id: number) => {
  try {
    const { data } = await axiosWithAuth.get(
      `/timeline-editor/get-timeline-item-detail`,
      {
        params: {
          timelineItemId: id,
        },
    ...
    notification.open({
      type: "error",
      message: `timeline`,
      description: "Something went wrong while fetching timeline card details",
    });
  }
};
...
export default function AddEditTimelineCard({ id, parentId }: IProps) {
  ...
  const onFinish = async (values: any) => {
    ...
    try {
      const res = await axiosWithAuth.post(
        "/timeline-editor/add-or-modify-timeline-item",
        modifiedValues
      );
      if (res.status == 200) {
        notification.open({
        ...
        Router.push(
          `/timeline/edit/${parentId || dataTimelineDetails.timelineId || id}`
        );
      }
    } catch (e: any) {
      console.log("e");
  ...
  const uploadImage = async (options: any) => {
    ...
    try {
      const res = await axiosWithAuth.post(
        `/timeline-editor/upload-timeline-detail-image`,
        formData,
        config
      );
      if (res.status == 200) {
  ...
  const getDefaultValue = () => {
    if (isEditPage) {
      const newData = {
        ...dataTimelineDetails,
        timelineDetails: dataTimelineDetails?.timelineDetails?.map(
          (detail: any) => ({
            ...detail,
          })
        ),
      ...
      return {
        ...
        timelineId: id,
        itemDate: null,
        timelineDetails: activeLanguages?.map((e) => {
          return {
            title: null,
            subTitle: null,
            timelineId: id,
    ...
    <div className={"p-2 pb-[60px]"}>
      <div className={"w-full flex justify-between items-center mb-4"}>
        ...
        <h2 className={"text-center text-[30px] w-full"}>
          {id ? "Edit Timeline Card" : "Add Timeline Card"}
        </h2>
      </div>
      <Divider className={"my-3"} />
      {((isEditPage && dataTimelineDetails) ||
        ...
        >
          ...
          <Form.Item name={"itemDate"} label={"Item Date"}>
            <Input placeholder="Item Date" />
          </Form.Item>

          <Form.List name="timelineDetails">
            {(fields, v) => {
              return (
