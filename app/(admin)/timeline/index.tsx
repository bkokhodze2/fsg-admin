...
export default function AddEditTimeline({ id }: IProps) {
  ...
  const fetchTimelineDetailsById = async (id: number) => {
    ...
    try {
      const { data } = await axiosWithAuth.get(
        `/timeline-editor/get-timeline-detail`,
        {
          params: {
            timelineId: id,
          },
      ...
      notification.open({
        type: "error",
        message: `timeline`,
        description: "Something went wrong while fetching timeline details",
      });
    }
  };
  const id2 = useId();
  ...
  const onFinish = async (values: any) => {
    ...
    try {
      const res = await axiosWithAuth.post(
        "timeline-editor/add-or-modify-timeline",
        modifiedValues
      );
  ...
  const postSortedData = async (sortedData: DataType[]) => {
    ...
    try {
      await axiosWithAuth.post("/timeline-editor/sort-timeline-items", {
        sortElements,
      });
    } catch (error) {
    ...
    if (active.id !== over?.id) {
      setDataSource((prev) => {
        const activeIndex = prev.findIndex(
          (item) => item.timelineItemId === active.id
        );
        const overIndex = prev.findIndex(
          (item) => item.timelineItemId === over?.id
        );
