...
const fetchPartnerDetailsById = async (id: number) => {
  try {
    const {data} = await axiosWithAuth.get(`/partner-editor/get-partner-item-detail`, {
      params: {
        partnerItemId: id
      }
    });
    ...
    notification.open({
      ...
      message: `partner`,
      description:
          'Something went wrong while fetching partner card details',
    });
  }
}
...
export default function AddEditPartner({id, parentId}: IProps) {
  ...
  const onFinish = async (values: any) => {
    ...
    try {
      const res = await axiosWithAuth.post('/partner-editor/add-or-modify-partner-item', modifiedValues)
      if (res.status == 200) {
        notification.open({
          type: 'success',
  ...
  const uploadImage = async (options: any) => {
    ...
    try {
      const res = await axiosWithAuth.post(`/partner-editor/upload-partner-item-image`, formData, config)
      if (res.status == 200) {
        onSuccess(res.data)
      }
    } catch (e: any) {
  ...
  const getDefaultValue = () => {
    if (isEditPage) {
      const newData = {
        ...dataPartnerDetails,
        partnerDetails: dataPartnerDetails?.partnerDetails?.map((detail: any) => ({
          ...detail,
        }))
      };
  ...
  let fileList = dataImg?.url ? [dataImg] : dataPartnerDetails?.imageData ? [dataPartnerDetails.imageData] :  [];
  ...
  return (
      <div className={"p-2 pb-[60px]"}>
            ...
            <h2 className={"text-center text-[30px] w-full"}>{id ? "Edit Partner" : "Add Partner"}</h2>
            </div>
