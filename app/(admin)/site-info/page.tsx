...
import { Button, Form, Input, Card, Divider, notification, Radio } from "antd";
...
export default function SiteInfoPage() {
  ...
  const fetchSiteInfoDetailsById = async (id: number) => {
    try {
      const { data } = await axiosWithAuth.get(
        `/site-info-editor/get-site-info-detail`,
        {
          params: {
            siteInfoId: id,
          },
      ...
      notification.open({
        type: "error",
        message: `site info`,
        description: "Something went wrong while fetching site info details",
      });
    }
  };
  ...
  const fetchSiteAdvInfos = async () => {
    try {
      const { data } = await axiosWithAuth.get(
        `${BASEAPI}/site-info-editor/get-site-adv-infos`
      );
      return data;
    } catch (error: any) {
      console.log("errr", error);
      notification.open({
        type: "error",
        message: `Info Adv`,
        description: "Something went wrong while fetching Info Adv items",
      });
    }
  };
  ...
  const { data: dataInfoAdv, refetch } = useQuery({
    queryKey: ["infos"],
    queryFn: fetchSiteAdvInfos,
    enabled: !!id,
  });
  ...
  const onFinish = async (values: any) => {
    ...
    try {
      const res = await axiosWithAuth.post(
        "/site-info-editor/add-or-modify-site-info-simple",
        modifiedValues
      );
      if (res.status == 200) {
        notification.open({
    ...
    <>
      ...
      <div className={"p-2 pb-[60px]"}>
        <div className="w-full">
          {((isEditPage && dataSiteInfoDetails) ||
            ...
            >
              <Form.List name="details">
                {(fields, v) => {
                  return (
                    <div className={"grid col-span-2 grid-cols-2 gap-x-5"}>
              ...
              <div
                className={"grid col-span-2 grid-cols-1 md:grid-cols-2 gap-x-5"}
              >
                <div className={""}>
                  <Form.Item
                    className={"mb-0"}
                  ...
                  <Form.Item name={"tenderEmail"} label={"Tender Email"}>
                  ...
                  <Form.Item name={"androidAppLink"} label={"Android App Link"}>
                    <Input placeholder="Android App Link" />
                  </Form.Item>

                  <Form.Item name={"iosAppLink"} label={"Ios App Link"}>
              ...
              <div className={"grid col-span-2"}>
                <Button
                  className={"mt-4 w-min mx-auto px-20"}
                  type={"primary"}
                  htmlType={"submit"}
                >
                  Submit
