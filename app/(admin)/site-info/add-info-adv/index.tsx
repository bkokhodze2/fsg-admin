...
const fetchCategories = async () => {
  try {
    const { data } = await axiosWithAuth.get(
      `${BASEAPI}/site-info-editor/get-site-info-adv-categories`
    );
    return data;
  } catch (error: any) {
...
const fetchInfoAdvDetailsById = async (id: number) => {
  try {
    const { data } = await axiosWithAuth.get(
      `/site-info-editor/get-site-info-adv-detail`,
      {
        params: {
          siteInfoAdvId: id,
        },
    ...
    notification.open({
      ...
      description: "Something went wrong while fetching site info card details",
    });
  }
};
...
export default function AddEditInfoAdv({ id }: IProps) {
  ...
  const onFinish = async (values: any) => {
    ...
    try {
      const res = await axiosWithAuth.post(
        "/site-info-editor/add-or-modify-site-info-adv",
        modifiedValues
      );
      if (res.status == 200) {
        notification.open({
  ...
  const uploadImage = async (options: any) => {
    ...
    try {
      const res = await axiosWithAuth.post(
        `/site-info-editor/upload-site-info-adv-file`,
        formData,
        config
      );
      if (res.status == 200) {
    ...
    <div className={"p-2 pb-[60px]"}>
      <div className={"w-full flex justify-between items-center mb-4"}>
        ...
        <h2 className={"text-center text-[30px] w-full"}>
          {id ? "Edit Site Info Adv" : "Add Site Info Adv"}
        </h2>
      </div>
      <Divider className={"my-3"} />
      {((isEditPage && dataInfoAdvDetails) ||
